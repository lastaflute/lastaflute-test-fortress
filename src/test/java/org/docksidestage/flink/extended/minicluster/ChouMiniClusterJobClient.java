 ...
 */
public final class ChouMiniClusterJobClient implements JobClient, CoordinationRequestGateway {

    private static final Logger LOG = LoggerFactory.getLogger(MiniClusterJobClient.class);

    private final JobID jobID;
    private final ChouMiniCluster miniCluster;
     ...
     */
    public ChouMiniClusterJobClient(JobID jobID, ChouMiniCluster miniCluster, ClassLoader classLoader,
        ...
        if (finalizationBehaviour == JobFinalizationBehavior.SHUTDOWN_CLUSTER) {
            ...
            jobResultFuture = miniCluster.requestJobResult(jobID).whenComplete((result, throwable) -> shutDownCluster(miniCluster));
        } else if (finalizationBehaviour == JobFinalizationBehavior.NOTHING) {
            jobResultFuture = miniCluster.requestJobResult(jobID);
        } else {
            throw new IllegalArgumentException("Unexpected shutdown behavior: " + finalizationBehaviour);
        }
    }
    ...
    public CompletableFuture<Map<String, Object>> getAccumulators() {
        ...
        if (jobExecutionResult.isDone()) {
            ...
            return miniCluster.getExecutionGraph(jobID)
    ...
    public CompletableFuture<CoordinationResponse> sendCoordinationRequest(String operatorUid, CoordinationRequest request) {
        try {
            ...
            return miniCluster.deliverCoordinationRequestToCoordinator(jobID, operatorUid, serializedRequest);
        } catch (IOException e) {
