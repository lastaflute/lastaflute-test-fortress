...
/**
 * The definition of harbor classification.
 * @author FreeGen
 */
public interface HarborCDef extends Classification {

    /**
     * Harbor's Member Status
     */
    public enum MemberStatus implements HarborCDef {
        /** ShowBase: Formalized */
        ...
        public ClassificationMeta meta() { return HarborCDef.DefMeta.MemberStatus; }
    ...
    public enum DefMeta implements ClassificationMeta {
        ...
        public OptionalThing<? extends Classification> of(Object code) {
            if (MemberStatus.name().equals(name())) { return HarborCDef.MemberStatus.of(code); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }

        public OptionalThing<? extends Classification> byName(String name) {
            if (MemberStatus.name().equals(name())) { return HarborCDef.MemberStatus.byName(name); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }

        public Classification codeOf(Object code) { // null if not found, old style so use of(code)
            if (MemberStatus.name().equals(name())) { return HarborCDef.MemberStatus.codeOf(code); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }

        public Classification nameOf(String name) { // null if not found, old style so use byName(name)
            if (MemberStatus.name().equals(name())) { return HarborCDef.MemberStatus.valueOf(name); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }

        public List<Classification> listAll() {
            if (MemberStatus.name().equals(name())) { return toClsList(HarborCDef.MemberStatus.listAll()); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }

        public List<Classification> listByGroup(String groupName) { // exception if not found
            if (MemberStatus.name().equals(name())) { return toClsList(HarborCDef.MemberStatus.listByGroup(groupName)); }
        ...
        public List<Classification> listOf(Collection<String> codeList) {
            if (MemberStatus.name().equals(name())) { return toClsList(HarborCDef.MemberStatus.listOf(codeList)); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }

        public List<Classification> groupOf(String groupName) { // old style
            if (MemberStatus.name().equals(name())) { return toClsList(HarborCDef.MemberStatus.groupOf(groupName)); }
            throw new IllegalStateException("Unknown definition: " + this); // basically unreachable
        }
        ...
        public static OptionalThing<HarborCDef.DefMeta> find(String classificationName) { // instead of valueOf()
            if (classificationName == null) { throw new IllegalArgumentException("The argument 'classificationName' should not be null."); }
            if (MemberStatus.name().equalsIgnoreCase(classificationName)) { return OptionalThing.of(HarborCDef.DefMeta.MemberStatus); }
            return OptionalThing.ofNullable(null, () -> {
                throw new ClassificationNotFoundException("Unknown classification: " + classificationName);
            });
        }

        public static HarborCDef.DefMeta meta(String classificationName) { // old style so use find(name)
            if (classificationName == null) { throw new IllegalArgumentException("The argument 'classificationName' should not be null."); }
            if (MemberStatus.name().equalsIgnoreCase(classificationName)) { return HarborCDef.DefMeta.MemberStatus; }
